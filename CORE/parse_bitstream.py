        ...
        if self.file_type == ".rbt":
            ...
            while cur_index < len(self.rbt_cfg_content_pre):
                if self.rbt_cfg_content_pre[cur_index].cmd_name == "COR0":
                    # 更新COR0
                    cor_data = self.rbt_cfg_content_pre[cur_index].get_data_from_index(1)
...
def run_bit_process(
    ...
    logging.info(f"\tfile_suffix: {file_suffix}")
    logging.info(f"\tPCIE: {pcie}")
    logging.info(f"\tGTP: {gtp}")
    ...
    bit_parser = BitstreamParser(dev, file, crc)

    if gtp:
        for item in GTP_CONFIG:
            bit_parser.set_data_with_frame_word_bit(
    ...
    out_path = bit_parser.save_file(file_suffix)
    logging.info("✔️ Done! Saved to %s", out_path)
    return out_path
